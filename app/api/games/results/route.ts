...
export async function GET(request:Request){
  ...
  const game=await db().prepare("SELECT g.status,g.actual_character_ids_json AS actualJson FROM games g JOIN organization_members m ON m.organization_id=g.organization_id WHERE g.id=? AND m.user_id=?").bind(gameId,user.id).first<{status:string;actualJson:string|null}>();
  ...
  const actual=JSON.parse(game.actualJson) as string[];const rows=await db().prepare('SELECT u.id AS userId,u.display_name AS displayName,p.character_ids_json AS characterIdsJson FROM predictions p JOIN users u ON u.id=p.user_id WHERE p.game_id=?').bind(gameId).all<{userId:string;displayName:string;characterIdsJson:string}>();

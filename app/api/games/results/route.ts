...
export async function GET(request: Request) {
  ...
  const game = await db()
    .prepare(
      'SELECT g.status,g.actual_character_ids_json AS actualJson FROM app_games g JOIN organization_members m ON m.organization_id=g.organization_id WHERE g.id=? AND m.user_id=?',
    )
    .bind(gameId, user.id)
    .first<{ status: string; actualJson: string | null }>();
  if (!game)
  ...
  const ranking = rows.results
    .map((row) => {
      ...
      return {
        ...
        characterIds,
        score: characterIds.filter((id) => actual.includes(id)).length,
      };
    })
    .sort(

...
export async function currentUser(request: Request) {
  ...
  return await db()
    .prepare(
      'SELECT u.id,u.username,u.display_name AS displayName FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.token_hash=? AND s.expires_at>?',
    )
...
export async function requireUser(request: Request) {
  ...
  if (!user)
    throw new Response(JSON.stringify({ error: 'Debes iniciar sesión.' }), {
      status: 401,
      headers: { 'content-type': 'application/json' },
    });
  return user;
}
export function isSuperAdmin(user: { username: string }) {
  const allowed = (process.env.SUPERADMIN_USERNAMES ?? '')
    .split(',')

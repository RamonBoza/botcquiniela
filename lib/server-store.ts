...
export async function ensureDatabase(){
  ...
  await d.batch([
    ...
    d.prepare("CREATE TABLE IF NOT EXISTS games (id TEXT PRIMARY KEY, code TEXT NOT NULL UNIQUE, name TEXT NOT NULL, organization_id TEXT NOT NULL, created_by TEXT NOT NULL, script_name TEXT NOT NULL, characters_json TEXT NOT NULL, player_count INTEGER NOT NULL, townsfolk_count INTEGER NOT NULL, outsider_count INTEGER NOT NULL, minion_count INTEGER NOT NULL, demon_count INTEGER NOT NULL, status TEXT NOT NULL DEFAULT 'OPEN', actual_character_ids_json TEXT, created_at INTEGER NOT NULL, FOREIGN KEY(organization_id) REFERENCES organizations(id))"),
    d.prepare('CREATE TABLE IF NOT EXISTS predictions (id TEXT PRIMARY KEY, game_id TEXT NOT NULL, user_id TEXT NOT NULL, character_ids_json TEXT NOT NULL, submitted_at INTEGER NOT NULL, updated_at INTEGER NOT NULL, UNIQUE(game_id,user_id), FOREIGN KEY(game_id) REFERENCES games(id), FOREIGN KEY(user_id) REFERENCES users(id))'),
    ...
    d.prepare('CREATE INDEX IF NOT EXISTS idx_games_organization ON games(organization_id)'),
  ]);
  initialized=true;
}
